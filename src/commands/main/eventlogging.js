...
module.exports = {
  ...
  async execute(interaction) {
    try {
      ...
      const eventChannel = interaction.client.channels.cache.get(
        "1002610487378321520"
      );
      const restrictedGuildIds = [
        "1313768451768188948",
        "1078478406745866271",
        "",
      ];
          ...
          return interaction.reply({
            embeds: [
              new EmbedBuilder()
                ...
                .setDescription(
                  "Please use this command in the main server (KOG)."
                )
                .setColor(Colors.Red),
            ],
            ephemeral: true,
      ...
      if (
        !memberRoles.has(mrId) &&
        !memberRoles.has(hrId) &&
        !memberRoles.has("875599118632878092")
      ) {
        return interaction.reply({
          embeds: [
            new EmbedBuilder()
      ...
      const eventSelectCollector =
        selectionMessage.createMessageComponentCollector({
          filter: (i) =>
            i.customId === "event_type" && i.user.id === interaction.user.id,
          time: 60000,
        });
      ...
      eventSelectCollector.on("collect", async (i) => {
        ...
        const attendeesEmbed = new EmbedBuilder()
          ...
          .setDescription(
            "Mention all the attendees and send the message. Then click **Done**.\n\nExample: <@123><@456>"
          )
          .setTimestamp();
        ...
        mentionCollector.on("collect", (message) => {
          ...
          if (!attendeeMessage && message.mentions.users.size > 0) {
            ...
            if (pingRegex.test(message.content.trim())) {
              ...
              message.reply({
                content:
                  "❌ Event log canceled: Please only send a message with user pings (e.g. <@123><@456>).",
                ephemeral: true,
              });
            }
          }
        ...
        buttonCollector.on("collect", async (btn) => {
          ...
          if (btn.customId === "cancel") {
            await btn.reply({
              content: "❌ Event log process canceled.",
              ephemeral: true,
            });
          ...
          if (!attendeeMessage || attendees.length === 0) {
            await btn.reply({
              content:
                "❌ Event log canceled: No valid attendee pings were provided.",
              ephemeral: true,
            });
            return cleanup(interactionMessages);
          }
          ...
          const meritEmbed = new EmbedBuilder()
            ...
            .setTitle("Merit")
            .setDescription(
              "Mention anyone who earned a merit or type **None**. Then click **Done**."
            )
            .setTimestamp();

          const meritButtons = new ActionRowBuilder().addComponents(
            new ButtonBuilder()
              .setCustomId("merit_done")
              .setLabel("Done")
              .setStyle(ButtonStyle.Primary),
            new ButtonBuilder()
              .setCustomId("merit_cancel")
              .setLabel("Cancel")
              .setStyle(ButtonStyle.Danger)
          );
          ...
          meritCollector.on("collect", (msg) => {
            ...
            msg.mentions.users.forEach((user) => {
              const mention = user.id;
              if (!meritAttendees.includes(mention))
                meritAttendees.push(mention);
            });
          });

          const meritButtonCollector =
            meritPrompt.createMessageComponentCollector({
          ...
          meritButtonCollector.on("collect", async (b) => {
            ...
            const finalEmbed = new EmbedBuilder()
              ...
              .addFields(
                ...
                {
                  name: "Merits",
                  value: meritAttendees.length
                    ? meritAttendees.map((id) => `<@${id}>`).join("\n")
                    : "None",
                }
              )
              .setTimestamp();
              ...
              for (const userId of attendees) {
                ...
                if (eventType === "Training") {
                  if (existingData) {
                    await MyData.updateOne(
                      { userId },
                      { $inc: { TrainingsAttended: 1 } }
                    );
                  } else {
                    await MyData.updateOne(
                      { userId },
                      { $set: { TrainingsAttended: 1 } }
                    );
                  }
                }
              }
