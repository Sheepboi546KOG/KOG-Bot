...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName("list").setDescription("Lists all current admins.")
    ),
  async execute(interaction) {
    ...
    try {
      if (subcommand === "add") {
        ...
        if (existingAdmin) {
          return interaction.reply({
            content: `${user.tag} is already an admin.`,
            ephemeral: true,
          });
        }
        ...
        interaction.reply({
          content: `${user.tag} has been added as an admin.`,
        });
        ...
        if (!existingAdmin) {
          return interaction.reply({
            content: `${user.tag} is not an admin.`,
            ephemeral: true,
          });
        }
        ...
        interaction.reply({
          content: `${user.tag} has been removed as an admin.`,
        });
      ...
      console.error(error);
      interaction.reply({
        content: "An error occurred while managing the admin schema.",
        ephemeral: true,
      });
    }
  },
